...
\begin{center}
 ...
 \begin{flushleft}
  {\small This paper difines the steganography technique to embed text information
  into gray scale image as the object of this studies. The method of
  embedding error pattern that is one of the embedding data method
  convert text information into error pattern that is more
  shorter and less hamming weight binary digit string and replace LSB
  plane in the image with the exclusive OR of error pattern and LSB
  plane in the image. In this study, we consider the image quality
  etc.. for the method of embedding error pattern. Then this paper also
  propose a method of dynamically generating the error pattern using the
  enumerative code.}
 \end{flushleft}
...
高まってきている．安全に通信を行う手段として主なものに送信内容の暗号化が
挙げられる．暗号化を施すことで，第三者による盗聴，なりすましといった妨害
行為を防ぐことができる．しかし，通信行為そのものは第三者に認知されてしま
うことから，通信経路の遮断などといった妨害行為をうける可能性を排除できな
い．

この点に対し，ステガノグラフィでは送信内容を別のデータ（これを「カバーデー
タ」という）に埋込むことで，第三者から秘密情報の存在そのものを隠蔽する．こ
れによって，通信行為そのものを隠蔽し，より安全な通信を実現することができ
る．

しかし，データの埋め込みはカバーデータ内で誤りビットを発生させ，それに伴
いカバーデータの劣化させてしまう．そのため，埋め込みによる誤りビットの発
生をいかにして抑えるかが重要となる．また，この誤りビットの発生頻度を「誤
り率」という．しかし，これまで埋め込みによる誤り率とその値に伴うカバーデー
タの劣化の実測値について述べた論文はない．

優れた誤り率を持つ手法として誤りパターン埋め込み法が知られており，文献
\cite{Paper01}では，この手法における誤り率の理論的下限曲線を示している．
しかし，誤り率の実測値については示されておらず，また実用面で重要となる埋
め込みによるカバーデータの劣化については述べられていない．
...
め込まれる情報は少量であり，外部から確認できることが望ましいため埋め込ま
れるデータそのものには大きな価値はない．

一方でステガノグラフィは秘密情報を隠蔽するために，秘密情報を別のデータ
内に埋め込む．ステガノグラフィでは電子透かし
とは逆に埋め込まれるデータそのものに大きな価値があり，秘密情報の埋
め込みそのものを他者に認知されないこと（これを隠蔽性という）が重要となる．
一般的に埋め込まれる情報は少量とは限らず，多量のデータの埋め込みは埋め込
み対象となる別のデータを劣化させてしまい，第三者に埋め込みを検知される危
険性が高まる．そのため，ステガノグラフィでは以下の2点を同時に満たすこと
が強く望まれる．
...
のメディアデータはいくらかの冗長性を有しており，これらを利用した埋め込み
が主流である．本論文で用いるメディアデータである画像では，画像中の各ピク
セルにおけるLSBなどの下位ビットが冗長性となる．これらのビットはそれぞれ
が持つ情報量が少なく，変化させたとしても視覚的変化がほとんど発生しない．
特にLSBは情報量が最も少ないことから，埋め込み利用されることが多い．実際，
この特徴を利用した埋め込み手法としてLSB法（\ref{sec:lsb_tecknique}章）が
提案されており，ステガノグラフィにおける最も一般的な埋込み手法として知ら
れている．
}
...
単純なアルゴリズムであり実装が容易である反面，埋め込み後に誤りビットが発
生しやすい．そのため，ステゴデータの画質が劣化しやすく，ステゴデータの隠
蔽性に問題を引き起こす可能性がある．

このような問題から，これまでLSB法における誤りビットの発生を改善した手法が多数
提案されてきた．それらの中でも本論文で扱う誤りパターン埋め込み法は，誤り
ビットの発生を大きく改善した手法として知られている．これについては次章で述べる．
...
を埋め込むことができる．また，誤り率は埋め込みによる誤りビットの発生頻度
を表しており，一般的に誤り率が高いほど画質が劣化する傾向にある．LSB法で
は埋め込み率は常に100％であるが，一方で誤りビットが発生しやすいことから誤
り率が高い．
}
...
使用し，埋め込みに使用するテキスト情報は等確率で発生する8bitコードの列と
...
\renewcommand{\labelenumi}{(\arabic{enumi})}
 ...
 \item 誤りパターン長を8bitから255bitまで変化させ（\ref{enum:experiment1-1}），（\ref{enum:experiment1-2}）を繰り返す．\label{enum:experiment1-3}
...
\subsubsection{2ビットプレーン以降の埋め込み\label{sec:more_bit_prane}}{
\ref{sec:result_ssim1}章の結果から，隠蔽性に影響を及ぼさない範囲で2ビットプレーン以降への埋め込みも可能なのではないかと考え，2〜4ビットプレーンまでの埋め込みを行った．実験手順はこれまでと
...
だ場合について，どのような違いが見られるかを検証するため，これまでと同様の
実験手順で比較を行った．
...
にこの結果を受けて，埋め込み範囲を広げてのSSIMの計測を行い，2ビットプレーン
まで使用することでほとんどの画像に対し1.9倍近くのテキスト情報の埋め込みが
