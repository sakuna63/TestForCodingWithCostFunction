...
\begin{center}
 ...
 \begin{flushleft}
  ...
  ノグラフィ技術を対象とする．データの埋め込み手法の一つである誤りパター
 ...
 \begin{flushleft}
  ...
  into gray scale images. The method of embedding error pattern is one
  of the steganography methods. In embedding process of this method,
  first of all, the binary data of  each characters in text information are converted into error
  pattern which has more longer and less hamming weight binary digit
  string than the binary data of character.  Finally, we replace LSB
  plane of the image with the result of calculating exclusive OR of
  error pattern and LSB plane of the image. In this study, we consider
  the effect (i.e., image quality degradation) by the method of
  embedding error pattern. Then this paper also proposes a method of
  dynamically generating the error pattern using the enumerative code
  proposed by Shalkwijk.
  }
 \end{flushleft}
...
ステガノグラフィでは送信内容を別のデータに埋め込み，第三者から通信行為の
存在そのものを隠蔽することで，より安全な通信を実現することができる．しか
し，データの埋め込みによって，埋め込みに使用されたデータ（これをカバーデー
タという）内に誤りビットが発生し，それに伴うカバーデータの劣化
が情報の隠蔽性に問題を引き起こしてしまう．この誤りビットが発生する割合を
誤り率という．カバーデータの劣化が目立たないようにデータを埋め込む必要が
あり，それに伴い埋め込み可能なデータ容量は制限されてしまう．

そのため，ステガノグラフィでは誤りを抑えつつ，より多くの情報を効率よく埋
め込むことが重要である．この埋め込みの効率を埋め込み率という．これまで複
数の埋め込み手法が提案されてきており，その中でも代表的なものがLSB法であ
る．文献\cite{Paper01}ではLSB法とそれを改良した誤りパターン埋め込み法に
ついての性能比較を行っているが，埋込率に対する誤り率の変化という観点から
しか比較を行っていない．ステガノグラフィの実用する上では埋め込みによるカ
バーデータの劣化について考慮する必要があり，誤り率のみでは埋め込み法の実
用面を評価するには不十分である．

そこで，本論文ではグレースケール画像にテキスト情報を埋
め込むステガノグラフィ技術を対象とし，誤りパターン埋め込み法における埋め込
み率と誤り率，画質劣化とのトレードオフ関係を実験的に明らかにすることで，
誤りパターン埋め込み法の画質劣化等について考察を行なう．なお，画質劣化の
指標にはSSIM\cite{Paper03}を用いる．また，誤りパターンテーブルの肥大化問
題\cite{Paper01}に対する解決策としてShalkwijkの数え上げ符号
...
証拠データの頑健さ，つまり他者に除去されないことが重要となる．一般的に埋
...
たとしても視覚的変化はほとんど発生しない．特にLSBは情報量が最も少ないこ
とから，埋め込み利用されることが多い．実際，この特徴を利用した埋め込み手
法としてLSB法が提案されており，ステガノグラフィにおける最も一般的な埋込
み手法として知られている．
}
...
テキスト情報の抽出時には図\ref{fig:lsb_ext}のように，ステゴデータのにはテ
キスト情報がそのまま埋め込まれていることから，特別な処理を行わずに抽出する
ことができる．
...
タの劣化に伴って隠蔽性に問題を引き起こしやすいという欠点がある．この欠点
...
は常に100％であるが，一方で誤りビットが発生しやすいことから誤り率が高く
なる．
}
...
\begin{figure}[htbp]
 \begin{center}
  \includegraphics[width=7.5cm]{img/lsb_error_theory.png}
 \end{center}
...
この手法では図\ref{fig:error_pt_emb}のように，テキスト情報を，冗長でより
ハミング重みの小さいビット列である誤りパターンへと変換する．
そして，カバービット列をそのまま置き換えるのではなく，カバービット列と誤
りパターンとの排他的論理和でカバービット列を置き換える．
...
率は減少する．また，テキスト情報から生成される誤りパターンのハミング重みとステゴビット列中で発生する誤りビット数が等しくなるため，同じテキスト情報を埋め込みデータとした場合，カバーデータによらず誤り率は等しくなる．
}
...
\section{カバーデータの劣化評価}
\subsection{評価手順} {
評価手順の概要を図\ref{fig:experiment1}に示す．以下に示す手順で実験を
行い，この結果について考察を行った．実
...
\begin{figure}[htbp]
 \begin{center}
  \includegraphics[width=7.5cm]{img/graph_error_rate.png}
 \end{center}
...
ないと言われていることから，カバービット列をLSB上のビットに限定した
...
であると仮定し，2〜4ビット列目までの埋め込みを行った．実験手順は
...
ていることがわかる．このことから，LSBへの埋め込みを100％として2ビット列
目まで埋め込みを行うことで，ほとんどの画像に対し，視覚的な隠蔽性に影
...
のみへの埋め込みに比べ3倍以上のテキスト情報の埋め込みが可能になると
いえる．
...
これについて，埋め込む文字数を固定し，LSBのみへ埋め込みを行った場合，
LSBに加え2ビット列目まで埋め込みを行った場合について，後者の場合により画質
...
のトレードオフ関係を実験的に明らかにした．これによって，カバービット列を
LSBに限定した場合，埋め込み率，誤り率によって画質が大きく劣化することはなく，
